...
app.post('/api/draw', async (req, res) => {
    ...
    } catch (e) { res.status(500).json({ success: false, message: '抽奖失败' }); }
});
...
app.get('/api/stats', async (req, res) => {
    try {
        ...
        res.json({ success: true, data: { totalDraws: t, byLevel: bl } });
...
app.get('/api/records', async (req, res) => {
    try {
        ...
        const { count: t } = await supabase.from('lottery_records').select('*', { count: 'exact', head: true });
        const { data: r } = await supabase.from('lottery_records').select('*').order('created_at', { ascending: false }).range(off, off + limit - 1);
        res.json({ success: true, data: { records: r || [], pagination: { page, limit, total: t || 0, totalPages: Math.ceil((t || 0) / limit) } } });
    } catch (e) { res.status(500).json({ success: false, message: '获取记录失败' }); }
});

app.post('/api/admin/login', (req, res) => {
    if (req.body.password === ADMIN_PASSWORD) { const t = uuidv4(); adminTokens.add(t); res.json({ success: true, token: t }); }
    else res.status(401).json({ success: false, message: '密码错误' });
});

const guard = (req, res, next) => {
    const t = req.headers['x-admin-token'] || req.query.token;
    if (t && adminTokens.has(t)) next(); else res.status(401).json({ success: false, message: '未授权' });
};
...
app.get('/api/admin/records', guard, async (req, res) => {
    try {
        const page = parseInt(req.query.page) || 1, limit = parseInt(req.query.limit) || 20, off = (page - 1) * limit;
        const { count: t } = await supabase.from('lottery_records').select('*', { count: 'exact', head: true });
        const { data: r } = await supabase.from('lottery_records').select('*').order('created_at', { ascending: false }).range(off, off + limit - 1);
        res.json({ success: true, data: { records: r || [], pagination: { page, limit, total: t || 0, totalPages: Math.ceil((t || 0) / limit) } } });
    } catch (e) { res.status(500).json({ success: false, message: '获取记录失败' }); }
...
app.get('/api/admin/export', guard, async (req, res) => {
    try {
        ...
        const wb = new ExcelJS.Workbook(), ws = wb.addWorksheet('抽奖记录');
        ws.columns = [
...
if (require.main === module) { app.listen(3000, () => console.log('Local: http://localhost:3000')); }
